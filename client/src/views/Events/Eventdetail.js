...
const Eventdetail = () => {
    ...
    const renderDataByYear = (year) => {
        ...
        const renderEventDataByYear = (year) => {
            if (year === '2026') {
                // Render import button for 2026
                return <Link to={`/EventData2026`}><button type="button" className="btn btn-primary"><RiTrophyLine /> Pick List</button></Link>;
            }
            else if (year === '2025') {
                // Render import button for 2025
                return <Link to={`/Eventdata`}><button type="button" className="btn btn-primary"><RiTrophyLine /> Pick List</button></Link>;
            }
            else {
                // Render import button for invalid years
                return <button type="button" className="btn btn-danger" disabled><RiAlertLine /> Pick List Not Available</button>;
            }
        };
